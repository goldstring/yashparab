/* ==========================================================================
   blog.js — blog listing page (blog.html)
   Plain jQuery. The posts live in the array below, so there is no JSON file to
   fetch and nothing to break when the page is opened straight from disk (file://).
   To add a post: add an object to `posts` (newest first). Set isActive: false to hide one.
   ========================================================================== */
$(function () {

  /* ---------- 1. DATA ---------- */
  var posts = [
    {
      title: "Why your feature store is the contract no one signed",
      category: "MLOps",
      date: "Sep 14, 2026",
      readTime: 9,
      excerpt: "Training and serving quietly disagree more often than anyone admits. Versioning the feature schema like an API fixed most of it for us.",
      image: "https://placehold.co/640x400/e87532/ffffff?text=Feature+Stores",
      alt: "Feature store architecture",
      link: "blog-detail.html",
      isActive: true
    },
    {
      title: "A canary rollout checklist for model deployments",
      category: "MLOps",
      date: "Sep 2, 2026",
      readTime: 7,
      excerpt: "The six checks I run before letting a new model version see more than 5% of production traffic.",
      image: "https://placehold.co/640x400/0f2943/ffffff?text=Canary+Rollouts",
      alt: "Canary deployment diagram",
      link: "blog-detail.html",
      isActive: true
    },
    {
      title: "Label noise is the silent killer of segmentation models",
      category: "Computer Vision",
      date: "Aug 24, 2026",
      readTime: 11,
      excerpt: "How we found that 8% of our \"ground truth\" masks were wrong — and what that cost us in mIoU.",
      image: "https://placehold.co/640x400/e87532/ffffff?text=Segmentation+Labels",
      alt: "Segmentation labeling example",
      link: "blog-detail.html",
      isActive: true
    },
    {
      title: "Fine-tuning vs. prompting for support ticket triage",
      category: "NLP",
      date: "Aug 15, 2026",
      readTime: 8,
      excerpt: "We tried both on the same dataset. Here's where a fine-tuned classifier still wins.",
      image: "https://placehold.co/640x400/0f2943/ffffff?text=Ticket+Triage",
      alt: "Text classification pipeline",
      link: "blog-detail.html",
      isActive: true
    },
    {
      title: "What I actually ask ML engineer candidates",
      category: "Career",
      date: "Aug 3, 2026",
      readTime: 6,
      excerpt: "Fewer LeetCode questions, more \"walk me through a time your model was wrong in production.\"",
      image: "https://placehold.co/640x400/e87532/ffffff?text=ML+Interviews",
      alt: "Whiteboard interview notes",
      link: "blog-detail.html",
      isActive: true
    },
    {
      title: "Alerting on drift without drowning the on-call rotation",
      category: "MLOps",
      date: "Jul 22, 2026",
      readTime: 10,
      excerpt: "Statistical drift tests are noisy. Here's the alerting layer we built on top to cut false pages by 70%.",
      image: "https://placehold.co/640x400/0f2943/ffffff?text=Drift+Alerts",
      alt: "Model drift monitoring dashboard",
      link: "blog-detail.html",
      isActive: true
    },
    {
      title: "Getting a segmentation model under 15ms on-device",
      category: "Computer Vision",
      date: "Jul 9, 2026",
      readTime: 9,
      excerpt: "Quantization, pruning and a smaller backbone — what actually moved the latency needle and what didn't.",
      image: "https://placehold.co/640x400/e87532/ffffff?text=Edge+Inference",
      alt: "Model running on edge device",
      link: "blog-detail.html",
      isActive: true
    },
    {
      title: "Evaluating RAG systems beyond \"it looks right\"",
      category: "NLP",
      date: "Jun 28, 2026",
      readTime: 12,
      excerpt: "A retrieval-quality and faithfulness rubric that caught regressions our eyeball tests missed.",
      image: "https://placehold.co/640x400/0f2943/ffffff?text=RAG+Evals",
      alt: "Retrieval augmented generation evaluation",
      link: "blog-detail.html",
      isActive: true
    },
    {
      title: "The one-on-one questions that actually unblock junior ML hires",
      category: "Career",
      date: "Jun 14, 2026",
      readTime: 5,
      excerpt: "Most blockers I mentor around aren't technical — they're about not knowing what \"done\" looks like.",
      image: "https://placehold.co/640x400/e87532/ffffff?text=Mentoring+Notes",
      alt: "Mentoring session notes",
      link: "blog-detail.html",
      isActive: true
    },
    {
      title: "Cutting our training bill in half without losing accuracy",
      category: "MLOps",
      date: "May 30, 2026",
      readTime: 8,
      excerpt: "Spot instances, mixed precision and a smarter data loader — the changes that actually paid off.",
      image: "https://placehold.co/640x400/0f2943/ffffff?text=Cost+Tuning",
      alt: "Cloud cost dashboard",
      link: "blog-detail.html",
      isActive: true
    },
    {
      title: "Benchmarking vector databases for retrieval latency",
      category: "NLP",
      image: "https://placehold.co/640x400/e87532/ffffff?text=Vector+DB",
      alt: "Benchmarking vector databases for retrieval latency",
      link: "blog-detail.html",
      isActive: false
    },
    {
      title: "Notes on LoRA adapters for vision transformers",
      category: "Computer Vision",
      image: "https://placehold.co/640x400/0f2943/ffffff?text=LoRA",
      alt: "Notes on LoRA adapters for vision transformers",
      link: "blog-detail.html",
      isActive: false
    }
  ];

  /* ---------- 2. SETTINGS ---------- */
  var PAGE_SIZE  = 9;                                              // cards per page
  var MIN_CHARS  = 3;                                              // search opens after this many characters
  var CATEGORIES = ['MLOps', 'Computer Vision', 'NLP', 'Career'];  // order of the filter chips
  function SEARCH_TEXT(p) { return p.title + ' ' + p.category; }   // what the search box looks at

  /* ---------- 3. PAGE TEXT ---------- */
  $('[data-list="crumb"]').text('Blog');
  $('[data-list="tag"]').text('Writing');
  $('[data-list="title"]').text('Notes on shipping ML that holds up.');
  $('[data-list="lead"]').text('Write-ups on computer vision, NLP and the MLOps plumbing that keeps models healthy after launch \u2014 mostly things I wish someone had told me before I learned them the hard way.');
  $('[data-list="searchLabel"]').text('Search articles');
  $('#listSearchInput').attr('placeholder', 'Search articles by title or topic (3+ characters)');
  $('#listEmptyText').text('No posts in this topic yet \u2014 pick another one.');

 

  /* ---------- helpers ---------- */
  function esc(s) {
    return String(s).replace(/[&<>"']/g, function (c) {
      return { '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[c];
    });
  }
  function escRe(s) { return s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'); }

  function chip(key, label, count, isActive) {
    return '<button type="button" class="filter-btn' + (isActive ? ' active' : '') + '" data-key="' + esc(key) +
           '" aria-pressed="' + isActive + '">' + esc(label) + ' <small>' + count + '</small></button>';
  }
  function setActive($wrap, key) {
    $wrap.find('.filter-btn').each(function () {
      var on = $(this).attr('data-key') === key;
      $(this).toggleClass('active', on).attr('aria-pressed', on);
    });
  }

  /* ---------- 4. CARD ---------- */
  function cardHtml(p) {
    return '<div class="blog-card">' +
      '<div class="blog-thumb">' +
        '<img src="' + esc(p.image) + '" alt="' + esc(p.alt) + '" loading="lazy">' +
        '<span class="blog-cat-badge">' + esc(p.category) + '</span>' +
      '</div>' +
      '<div class="blog-card-body">' +
        '<div class="blog-date-row"><span>' + esc(p.date) + '</span><span class="dot"></span><span>' + esc(p.readTime) + ' min read</span></div>' +
        '<h4><a href="' + esc(p.link) + '">' + esc(p.title) + '</a></h4>' +
        '<p>' + esc(p.excerpt) + '</p>' +
        '<a href="' + esc(p.link) + '" class="blog-read-link">Read article <i class="bi bi-arrow-right"></i></a>' +
      '</div>' +
    '</div>';
  }

  /* ---------- 5. FILTER CHIPS ---------- */
  var visible = posts.filter(function (p) { return p.isActive; });
  var state = { cat: 'all', page: 1 };

  var chips = chip('all', 'All posts', visible.length, true);
  CATEGORIES.forEach(function (c) {
    chips += chip(c, c, visible.filter(function (p) { return p.category === c; }).length, false);
  });
  $('#listCats').html(chips).on('click', '.filter-btn', function () {
    state.cat = $(this).attr('data-key');
    state.page = 1;
    setActive($('#listCats'), state.cat);
    render();
  });

  /* ---------- 6. DRAW THE LIST ---------- */
  function render() {
    var list = visible.filter(function (p) { return state.cat === 'all' || p.category === state.cat; });
    var pages = Math.max(1, Math.ceil(list.length / PAGE_SIZE));
    state.page = Math.min(state.page, pages);
    var from = (state.page - 1) * PAGE_SIZE;
    var slice = list.slice(from, from + PAGE_SIZE);

    $('#listGrid').html(slice.map(function (p) {
      return '<div class="col-md-6 col-lg-4 list-item">' + cardHtml(p) + '</div>';
    }).join(''));
    $('#listEmpty').toggleClass('show', list.length === 0);

    var noun = list.length === 1 ? 'post' : 'posts';
    $('#listCount').text(!list.length ? '' :
      pages === 1 ? list.length + ' ' + noun :
      'Showing ' + (from + 1) + '\u2013' + (from + slice.length) + ' of ' + list.length + ' ' + noun);

    drawPager(pages);
  }

  /* ---------- 7. PAGINATION ---------- */

  // Which page numbers to show:  1  2  3  ...  (last-2)  (last-1)  last
  // With 7 pages or fewer, every page is shown. If the current page is hidden in the
  // middle, it is shown too, so the active page never disappears.
  function pageNumbers(pages, current) {
    var show = [], i;
    if (pages <= 7) {
      for (i = 1; i <= pages; i++) show.push(i);
      return show;
    }
    show = [1, 2, 3, pages - 2, pages - 1, pages];
    if (current > 3 && current < pages - 2) show.push(current);
    show.sort(function (a, b) { return a - b; });

    var out = [];
    show.forEach(function (n, k) {
      if (k > 0 && n - show[k - 1] > 1) out.push('\u2026');      // gap in the numbers -> "..."
      out.push(n);
    });
    return out;
  }

  function drawPager(pages) {
    if (pages <= 1) { $('#listPager').empty().prop('hidden', true); return; }
    var p = state.page;
    var html = p > 1
      ? '<a href="#" class="pg-arrow" data-page="' + (p - 1) + '" aria-label="Previous page"><i class="bi bi-chevron-left"></i></a>'
      : '<span class="pg-arrow is-disabled" aria-hidden="true"><i class="bi bi-chevron-left"></i></span>';
    pageNumbers(pages, p).forEach(function (n) {
      if (n === '\u2026') {
        html += '<span class="pg-arrow pg-dots" aria-hidden="true">\u2026</span>';      // the "..." (borderless, like the arrows)
      } else if (n === p) {
        html += '<span class="active" aria-current="page">' + n + '</span>';
      } else {
        html += '<a href="#" data-page="' + n + '" aria-label="Page ' + n + '">' + n + '</a>';
      }
    });
    html += p < pages
      ? '<a href="#" class="pg-arrow" data-page="' + (p + 1) + '" aria-label="Next page"><i class="bi bi-chevron-right"></i></a>'
      : '<span class="pg-arrow is-disabled" aria-hidden="true"><i class="bi bi-chevron-right"></i></span>';
    $('#listPager').html(html).prop('hidden', false);
  }

  $('#listPager').on('click', 'a[data-page]', function (e) {
    e.preventDefault();
    state.page = parseInt($(this).attr('data-page'), 10);
    render();
    var top = $('.list-toolbar').offset().top - 110;              // bring the top of the list back into view
    if ($(window).scrollTop() > top) $('html, body').animate({ scrollTop: top }, 300);
  });


  /* ---------- 8. SEARCH BOX (opens after 3+ characters) ---------- */
  var $search  = $('#listSearch');
  var $input   = $('#listSearchInput');
  var $results = $('#listSearchList');
  var active   = -1;                        // highlighted result row

  function closeSearch() {
    $results.prop('hidden', true).empty();
    $input.attr('aria-expanded', 'false').removeAttr('aria-activedescendant');
    active = -1;
  }

  function openSearch() {
    var term = $.trim($input.val());
    if (term.length < MIN_CHARS) { closeSearch(); return; }

    var words = term.toLowerCase().split(/\s+/);
    var hits = visible.filter(function (p) {
      var text = SEARCH_TEXT(p).toLowerCase();
      return words.every(function (w) { return text.indexOf(w) !== -1; });
    });

    var html;
    if (!hits.length) {
      html = '<li role="presentation" class="list-search-empty"><i class="bi bi-search"></i>No results found for \u201c' + esc(term) + '\u201d</li>';
    } else {
      var re = new RegExp('(' + words.sort(function (a, b) { return b.length - a.length; }).map(escRe).join('|') + ')', 'i');
      html = hits.map(function (p, i) {
        var title = p.title.split(re).map(function (part, n) {
          return n % 2 ? '<mark>' + esc(part) + '</mark>' : esc(part);
        }).join('');
        return '<li role="presentation"><a class="list-search-result" id="listResult-' + i + '" role="option" aria-selected="false" href="' + esc(p.link) + '">' +
                 '<img src="' + esc(p.image) + '" alt="" width="64" height="44" loading="lazy">' +
                 '<span class="result-title">' + title + '</span>' +
                 '<span class="result-cat">' + esc(p.category) + '</span></a></li>';
      }).join('');
    }
    active = -1;
    $input.removeAttr('aria-activedescendant').attr('aria-expanded', 'true');
    $results.html(html).prop('hidden', false).scrollTop(0);
    $('#listSearchStatus').text(hits.length ? hits.length + ' results found' : 'No results found');
  }

  function highlightRow(i) {
    var $rows = $results.find('.list-search-result');
    if (!$rows.length) return;
    active = (i + $rows.length) % $rows.length;                    // wraps top <-> bottom
    $rows.removeClass('is-active').attr('aria-selected', 'false');
    var $row = $rows.eq(active).addClass('is-active').attr('aria-selected', 'true');
    $input.attr('aria-activedescendant', $row.attr('id'));
    var row = $row[0], box = $results[0];                          // keep the row inside the scroll area
    if (row.offsetTop < box.scrollTop) box.scrollTop = row.offsetTop;
    else if (row.offsetTop + row.offsetHeight > box.scrollTop + box.clientHeight) box.scrollTop = row.offsetTop + row.offsetHeight - box.clientHeight;
  }

  $input.on('input', function () {
    $search.toggleClass('has-value', $input.val().length > 0);
    openSearch();
  });
  $input.on('focus', function () { if ($results.prop('hidden')) openSearch(); });
  $input.on('keydown', function (e) {
    var isOpen = !$results.prop('hidden');
    if (e.key === 'ArrowDown') { e.preventDefault(); if (!isOpen) openSearch(); highlightRow(active + 1); }
    else if (e.key === 'ArrowUp') { e.preventDefault(); if (!isOpen) openSearch(); highlightRow(active < 0 ? -1 : active - 1); }
    else if (e.key === 'Enter' && isOpen && active >= 0) { e.preventDefault(); $results.find('.list-search-result').get(active).click(); }
    else if (e.key === 'Escape' && isOpen) { e.preventDefault(); closeSearch(); }
    else if (e.key === 'Tab') { closeSearch(); }
  });
  $results.on('mousemove', '.list-search-result', function () {
    var i = $results.find('.list-search-result').index(this);
    if (i !== active) highlightRow(i);
  });
  $('#listSearchClear').on('click', function () {
    $input.val('').focus();
    $search.removeClass('has-value');
    closeSearch();
  });
  $(document).on('click', function (e) {
    if (!$(e.target).closest('#listSearch').length) closeSearch();
  });

  render();
});